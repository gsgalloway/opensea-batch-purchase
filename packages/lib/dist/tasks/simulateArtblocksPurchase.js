...
(0, config_1.task)("simulate-artblocks-purchase")
    ...
    .setAction(async ({ gnosisSafeTxHash, tokenIds: tokenIDs }, hre) => {
    var _a, _b;
    const { ethers } = hre;
...
const checkOwnershipOfTokens = async (hre, safeAddr, tokenIDs) => {
    const { ethers } = hre;
    const artBlocks = await ethers.getContractAt("GenArt721", artblocksContractAddress);
